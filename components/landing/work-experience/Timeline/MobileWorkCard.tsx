  ...
  forwardRef<HTMLDivElement, MobileWorkCardProps>(
    ({ group, index, isExpanded, onToggle }, ref) => {
      ...
      return (
        ...
        >
          <div
            className={`absolute inset-0 bg-gradient-accent rounded-2xl ${
              isExpanded ? "opacity-100" : "opacity-0"
            }`}
          />

          <div
            className={`relative bg-background rounded-[15px] overflow-hidden ${
              ...
              <div className="flex items-center justify-between">
                ...
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center transition-transform duration-150 ${
                    isExpanded
                      ? "bg-foreground/10 rotate-180"
                      : "bg-foreground/5 rotate-0"
                  }`}
                >

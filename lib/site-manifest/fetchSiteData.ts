...
function partitionVisibleBlogs(allBlogs: Blog[]): Pick<SiteManifestData, 'projects' | 'blogPosts'> {
...
export async function fetchSiteData(db: Db): Promise<SiteManifestData> {
    const [configRows, workRows, visibleBlogs, certificationRows, contactRows] = await Promise.all([
        db.select().from(siteConfig).limit(1),
        db
            .select()
            .from(workExperiences)
            .orderBy(asc(workExperiences.order), desc(workExperiences.createdAt)),
        db.select().from(blogs).where(visibleBlogsFilter()).orderBy(desc(blogs.date)),

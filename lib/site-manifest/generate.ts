...
export function buildSitemapEntries(data: SiteManifestData): SitemapEntry[] {
    const now = new Date();
    const blogPages: SitemapEntry[] = [...data.projects, ...data.blogPosts].map((entry) => ({
...
function aboutText(data: SiteManifestData): { intro: string; experience: string; personal: string; outro: string } {
...
function formatCertifications(data: SiteManifestData): string {
    ...
    const items = data.certifications.map(
        (cert) => `- **${cert.title}** - ${cert.issuer} (${cert.date}): ${cert.description}${cert.link ? ` — ${cert.link}` : ''}`
    );
...
function formatContact(data: SiteManifestData): string {
    const contactLines =
        data.contacts.length > 0
            ? data.contacts.map((contact) => `- **${contact.label}:** ${contact.href}`)
            : [
...
export function buildLlmsTxt(data: SiteManifestData, siteDescription: string): string {
